...
void log_init() {
  ...
  data->records =
      (_log_record *)malloc(data->max_records * sizeof(struct _log_record));
...
void _log_internal(const int prio, const int prio_c, const char *prio_s,
                   const char *fmt, ...) {
  va_list ap;
  if (prio >= data->pri) {
    va_start(ap, fmt);
    char *fmt_modified = (char *)malloc(32 + strlen(fmt));
    snprintf(fmt_modified, 32 + strlen(fmt),
             "\x1b[1m\x1b[38;5;%dm %s \x1b[0m %s", prio_c, prio_s, fmt);
    ...
    vsnprintf(data->records[data->record_ptr].buf, data->record_bufsize, fmt,
              ap);
    data->records[data->record_ptr]
        .buf[strcspn(data->records[data->record_ptr].buf, "\n")] = 0;
...
int fetch_n_logs(int n, struct _log_record *buf) {
  int fetched = 0;
  int i = ((data->record_ptr - 1) % data->max_records + data->max_records) %
          data->max_records;
