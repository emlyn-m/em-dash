...
// never churn the network or flap ECONN. Control commands are enqueued (callable
// from anywhere, e.g. a future "mirror" feature) and applied in order once
// connected. Every reply/push the device sends is read and its dps folded into
// the model, so state stays live without correlating replies to commands.
namespace ui {
...
// Invoked on the main thread whenever the device's state changes. nullptr clears.
void led_on_update(int idx, std::function<void()> cb);

// Enqueue commands to the device's comm thread. Non-blocking; sent in order once
// connected. Callable from any thread.
