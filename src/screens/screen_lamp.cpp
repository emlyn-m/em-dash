...
static void set_ctrl_handler(GtkButton *_button, GdkEvent *_event,
                             void *data_v) {
...
struct _tuya_send_args {
  tuya_led_t *led;
  char *cmdbuf;
};
static void *_tuya_send_thread_helper(void *args_vp) {
  _tuya_send_args *args = (_tuya_send_args *)args_vp;
  tuya_cmd_send(args->led, COMMAND_CTRL, args->cmdbuf);
  free(args->cmdbuf);
  free(args);
  return NULL;
}
static void _tuya_color_set(tuya_led_t *led, uint32_t hue, uint32_t sat,
                            uint32_t val) {
  char *cmdbuf = (char *)malloc(32 * sizeof(char));
  memset(cmdbuf, 0, 32);
  led->hue = hue;
  led->sat = sat;
  led->val = val;
  snprintf(cmdbuf, 32, "{\"24\": \"%04x%04x%04x\"}", led->hue, led->sat,
           led->val);
  struct _tuya_send_args *args =
      (struct _tuya_send_args *)malloc(sizeof(struct _tuya_send_args));
  args->led = led;
  args->cmdbuf = cmdbuf;
  pthread_t thread_id;
...
static void led_power_callback(GtkButton *_button, GdkEvent *_event,
                               void *led_vp) {
...
static void led_hue_callback(float progress, void *led_vp) {
  tuya_led_t *led = (tuya_led_t *)led_vp;
  led->graph_control = 0;
  _tuya_color_set(led, progress * 359.0, led->sat, led->val);
}
static void led_sat_callback(float progress, void *led_vp) {
  tuya_led_t *led = (tuya_led_t *)led_vp;
  led->graph_control = 0;
  _tuya_color_set(led, led->hue, progress * 1000.0, led->val);
}
static void led_val_callback(float progress, void *led_vp) {
  tuya_led_t *led = (tuya_led_t *)led_vp;
  led->graph_control = 0;
  _tuya_color_set(led, led->hue, led->sat, progress * 1000.0);
}

struct _spawn_slider_args {
  tuya_led_t *led;
...
static void _update_values_from_buf(char *buf,
  ...
  if ((dp24 = cJSON_GetObjectItem(dps, "24"))) {
    sscanf(dp24->valuestring, "%04x%04x%04x", &(args->led->hue),
           &(args->led->sat), &(args->led->val));
    args->sliders[0]->value = ((float)args->led->hue) / 359.0;
    gtk_widget_queue_draw(args->sliders[0]->drawing_area);
    args->sliders[1]->value = ((float)args->led->sat) / 1000.0;
    gtk_widget_queue_draw(args->sliders[1]->drawing_area);
    args->sliders[2]->value = ((float)args->led->val) / 1000.0;
    gtk_widget_queue_draw(args->sliders[2]->drawing_area);
  }
...
static void *_spawn_led_strip_thread(void *args_vp) {
  ...
  if (status) {
    if (status == ERR_SOCK_FAIL) {
      LOG(PRI_ERR, "socket fail!\n");
    } else if (status == ERR_SOCK_CLOSE) {
      LOG(PRI_ERR, "\nsocket closed!\n");
    } else {
      LOG(PRI_ERR, "error code %d in tuya_recv_msg!\n", status);
    }
  } else {
...
struct _tuya_power_btn_args {
  tuya_led_t *led;
  GtkWidget *widget;
};
static gboolean power_button_update(void *data_vp) {
  struct _tuya_power_btn_args *data = (struct _tuya_power_btn_args *)data_vp;
  gtk_label_set_text(GTK_LABEL(data->widget), data->led->failures ? "!ECONN"
                                              : data->led->power  ? "/running"
                                                                  : "/paused");
  return TRUE;
}

static void set_mode_handler(GtkButton *_button, GdkEvent *_event,
                             void *led_vp) {
  tuya_led_t *led = (tuya_led_t *)led_vp;
  led->graph_control = 1 - led->graph_control;
}

static gboolean mode_handler_update(void *data_vp) {
  struct _tuya_power_btn_args *data = (struct _tuya_power_btn_args *)data_vp;
...
GtkWidget *generate_lamp_screen(GtkWidget *stack,
  ...
  struct _tuya_power_btn_args *pb_args = (struct _tuya_power_btn_args *)malloc(
  ...
  kindle_slider_t *hue_slider =
      kindle_slider_new(led, NULL, &led_hue_callback, 0);
  gtk_table_add(table, 0, 1, 1, 6, hue_slider->drawing_area);
  struct _img_src_dat *hue_dat =
  ...
  kindle_slider_t *sat_slider =
      kindle_slider_new(led, NULL, &led_sat_callback, 0);
  gtk_table_add(table, 1, 2, 1, 6, sat_slider->drawing_area);
  struct _img_src_dat *sat_dat =
      (struct _img_src_dat *)malloc(sizeof(struct _img_src_dat));
  ...
  kindle_slider_t *val_slider =
      kindle_slider_new(led, NULL, &led_val_callback, 0);
  gtk_table_add(table, 2, 3, 1, 6, val_slider->drawing_area);
  struct _img_src_dat *val_dat =
      (struct _img_src_dat *)malloc(sizeof(struct _img_src_dat));
  ...
  struct _spawn_slider_args *args =
      (struct _spawn_slider_args *)malloc(sizeof(struct _spawn_slider_args));
  args->led = led;
