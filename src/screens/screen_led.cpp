...
void led_hue_callback(float progress, void* led_vp) { tuya_led_t* led = (tuya_led_t*) led_vp; _tuya_color_set(led, progress * 360.0, led->sat, led->val); }
...
void _update_values_from_buf(char* buf, struct _spawn_slider_args* args) {
    ...
    if ((dp24 = cJSON_GetObjectItem(dps, "24"))) {
        ...
        args->sliders[0]->value = ((float) args->led->hue) / 360.0; gtk_widget_queue_draw(args->sliders[0]->drawing_area); 
