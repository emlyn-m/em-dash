...
gboolean clock_update(void *data_p) {
  ...
  if (tm_struct.tm_hour < 12) {
    snprintf(ampm_buf, 3, "am");
  } else {
    snprintf(ampm_buf, 3, "pm");
  }
...
GtkWidget *time_widget() {
  ...
  // wrapper
  PangoFontDescription *font_desc_month =
      pango_font_description_from_string(FONT_12);
  PangoFontDescription *font_desc_day =
      pango_font_description_from_string(FONT_20);
  PangoFontDescription *font_desc_ampm =
      pango_font_description_from_string(FONT_12);
  PangoFontDescription *font_desc_time =
      pango_font_description_from_string(FONT_16);
  ...
  GtkWidget *alignment =
      gtk_alignment_new(0.5, 0.5, 0.0, 0.0); // Center the child, do not scale
  ...
  g_timeout_add(get_attr_long("UI_UPDATE_FREQUENCY"), (GSourceFunc)clock_update,
                data);
