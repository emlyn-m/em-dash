...
constexpr int SHADOW = 0; // hard offset drop shadow, in px

inline cairo_t *begin_paint(GtkWidget *w) {
  return gdk_cairo_create(w->window);
}
...
template <typename T> T *attach(GtkWidget *w, const T &value) {
  T *p = g_new0(T, 1);
  *p = value;
