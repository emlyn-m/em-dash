...
void read_keyboard(kb_data_t* data) {
	...
	while (1) {
		...
		if (ev.type == EV_ABS) {
			if (ev.code == 53) {
				// abs-x
				if ((ev.value < X_MIN) || (ev.value > X_MAX)) { continue; }
				data->x = (((double) ev.value) - X_MIN) / (X_MAX - X_MIN);
				// printf("\x1b[38;5;139m\x1b[1mINFO\x1b[0m ev_x %d (%lf)\n", ev.value, data->x);
			} else if (ev.code == 54) {
				// abs-y
				if ((ev.value < Y_MIN) || (ev.value > Y_MAX)) { continue; }
			...
			if (ev.code == 325) {
				...
				if (ev.value == 1) {
					...
					if (pressed_key < 0x04) {
						data->layer = pressed_key;
						continue;
					}
					*(kb_buf_ptr++) = pressed_key;
					data->on_key_press(data);
				}
			}
		}
	}
...
void append_keycode_lut(kb_lut_t** kb_lut, int layer, double x, double y, double w, double h, char value) {
	printf("loading key <%c> in range (%lf, %lf) to (%lf, %lf) on layer %d\n", value, x, y, x+w, y+h, layer);
	kb_lut_t* lut = (kb_lut_t*) malloc(sizeof(kb_lut));
	if (*kb_lut) {
		(*kb_lut)->next = lut;
	} else { (*kb_lut) = lut; }
...
*/
	...
	append_keycode_lut(kb_lut, 0, 0   , 0.5 , 0.15, 0.25, 0x1); // lshift - goto layer 1
	...
	append_keycode_lut(kb_lut, 0, 0   , 0.75, 0.15, 0.25, 0x2); // goto layer 2
    ...
    append_keycode_lut(kb_lut, 1, 0   , 0.5 , 0.15, 0.25, 0x0); // lshift off - goto layer 0
    ...
    append_keycode_lut(kb_lut, 1, 0   , 0.75, 0.15, 0.25, 0x2); // goto layer 2
	...
	append_keycode_lut(kb_lut, 2, 0   , 0.5 , 0.15, 0.25, 0x3); // goto layer 3
	...
	append_keycode_lut(kb_lut, 2, 0   , 0.75, 0.15, 0.25, 0x0); // goto layer 0
	...
	append_keycode_lut(kb_lut, 3, 0   , 0.5 , 0.15, 0.25, 0x2); // goto layer 2
	...
	append_keycode_lut(kb_lut, 3, 0   , 0.75, 0.15, 0.25, 0x0); // goto layer 0
...
GtkWidget* keyboard_widget(void (*onpress)(kb_data_t* data), void (*onenter)(kb_data_t* data)) {
	...
	// display_keycode_lut(data->lookup_table);
