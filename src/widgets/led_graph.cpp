...
void button_callback_a(GtkButton* _b, GdkEvent* _e, void* gdata_vp) {
    for (int i=0; i < 12; i++) {
        ((led_graph_data_t*) gdata_vp)->points[i].v = &(((led_graph_data_t*) gdata_vp)->points[i]._v_h);
        ((led_graph_data_t*) gdata_vp)->points[i].sliderref->value = *((led_graph_data_t*) gdata_vp)->points[i].v;
    }
    gtk_widget_queue_draw(((led_graph_data_t*) gdata_vp)->drawref);
}
void button_callback_r(GtkButton* _b, GdkEvent* _e, void* gdata_vp) {
    for (int i=0; i < 12; i++) {
        ((led_graph_data_t*) gdata_vp)->points[i].v = &(((led_graph_data_t*) gdata_vp)->points[i]._v_s);
        ((led_graph_data_t*) gdata_vp)->points[i].sliderref->value = *((led_graph_data_t*) gdata_vp)->points[i].v;
    }
    gtk_widget_queue_draw(((led_graph_data_t*) gdata_vp)->drawref);
}
void button_callback_g(GtkButton* _b, GdkEvent* _e, void* gdata_vp) {
    for (int i=0; i < 12; i++) {
        ((led_graph_data_t*) gdata_vp)->points[i].v = &(((led_graph_data_t*) gdata_vp)->points[i]._v_v);
        ((led_graph_data_t*) gdata_vp)->points[i].sliderref->value = *((led_graph_data_t*) gdata_vp)->points[i].v;
    }
    gtk_widget_queue_draw(((led_graph_data_t*) gdata_vp)->drawref);
}

static void bezier_extrema_axis(float p0, float p1, float p2, float p3, float *out_min, float *out_max) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        float u = 1.0f - t;
        float v = u*u*u*p0 + 3.0f*u*u*t*p1 + 3.0f*u*t*t*p2 + t*t*t*p3;
        if (v < lo) lo = v;
        if (v > hi) hi = v;
    }
...
float getlevel_1(float x, float y) { return 0.3; }
float getlevel_2(float x, float y) { return 0.6; }
float getlevel_3(float x, float y) { return 1.0; }

void draw_lines(cairo_t *cr, led_graph_data_t* gdata, int w, int h) {
    
    int offset = 0;
    float(*levels[3])(float,float) = { &getlevel_1, &getlevel_2, &getlevel_3 };
    float offsets[3] = { .1, .3, .1 };
    cairo_save(cr);
    for (int i=0; i < 3; i++) {
        ...
        for (int j = 0; j < 12; j++) {
            ...
            int y1 = h - ((int) ( (*gdata->points[j].v - ((int) *gdata->points[j].v) ) * (float)(h - (lg_t + lg_b)) )) - lg_b + offset;
            if (j == 0) { cairo_move_to(cr, x1, y1); }
            if (j < 11) {
                int x0 = (int) ( ((float) (j-0.5) / 12.0) * (w) );
                int y0 = h - ((int) ( (*gdata->points[j > 0 ? j-1 : 0].v - ((int) *gdata->points[j > 0 ? j-1 : 0].v)) * (float)(h - (lg_t + lg_b)) )) - lg_b + offset;
    
                int x2 = (int) ( ((float) (j+1.5) / 12.0) * (w) );
                int y2 = h - ((int) ( (*gdata->points[j+1].v - ((int) *gdata->points[j+1].v)) * (float)(h - (lg_t + lg_b)) )) - lg_b + offset;
    
                int x3 = (int) ( ((float) (j+2.5) / 12.0) * (w) );
                int y3 = h - ((int) ( (*gdata->points[j < 10 ? j+2 : 0].v - ((int) *gdata->points[j < 10 ? j+2 : 0].v)) * (float)(h - (lg_t + lg_b)) )) - lg_b + offset;
                ...
                bezier_extrema_axis((float)y1, (float)yc1, (float)yc2, (float)y2, &cby_min, &by_max);
                if (cby_min < by_min) { by_min = cby_min; }
            }
        }
        if (i == 0) { cairo_stroke_preserve(cr); }
        ...
        cairo_set_source_rgba(cr, (2.-((float) i))/5.,(2.-((float) i))/5.,(2.-((float) i))/5.,1);
        crosshatch_bb(cr, w, h, 10, levels[i]);
...
void process_graph_update(led_graph_data_t* gdata) {
    ...
    req_a.width += LABEL_PAD; req_a.height += LABEL_PAD;
    ...
    for (int i = 0; i < 12; i++) {
        ...
        a = {
            (int) ( ((float) i / 13.0) * w ) + ledg_padw + 2, ledg_padh + lg_t,
            (w - 2*ledg_padw) / 12 - 4, h - ((int) 1.5*ledg_padh + lreq.height + lg_b + lg_t)
        };
        gtk_widget_size_allocate(gdata->points[i].sliderref->drawing_area, &a);
    }
    
    a = { ledg_padw, ledg_padh, w - 2*ledg_padw, h - ((int) 1.5*ledg_padh + lreq.height) };
    gtk_widget_size_allocate(gdata->drawref, &a);
    a = { ox + w - req_a.width - ledg_padw, oy + ledg_padh, req_a.width, req_a.height };
    gtk_widget_size_allocate(gdata->label_a, &a);
    a = { ox + w - req_a.width - ledg_padw, oy + ledg_padh + req_a.height + ledg_gaph, req_a.width, req_a.height };
    gtk_widget_size_allocate(gdata->label_r, &a);
    a = { ox + w - req_a.width - ledg_padw, oy + ledg_padh + req_a.height + ledg_gaph + req_a.height + ledg_gaph, req_a.width, req_a.height };
...
static gboolean _shadow_expose(GtkWidget* widget, GdkEventExpose* event, gpointer _data) {
	cairo_t* cr = gdk_cairo_create(widget->window);
...
GtkWidget* table_wrapping(GtkWidget* c) {
	...
	g_signal_connect(event_box_shadow, "expose-event", G_CALLBACK(_shadow_expose), NULL);
...
void slider_release(float v, void* data_vp) {
    graph_point_t* data = (graph_point_t*) data_vp;
    if (data->v == &(data->_v_tmp)) { data->v = &(data->_v_h) + (int) data->_v_tmp; }
    *(data->v) = MIN(MAX(0, v), 0.9999f);
}

GtkWidget* led_graph() {
    led_graph_data_t* gdata = (led_graph_data_t*) malloc(sizeof(led_graph_data_t));
    
    gdata->ref = gtk_fixed_new();
    gdata->drawref = gtk_drawing_area_new();
    gtk_fixed_put(GTK_FIXED(gdata->ref), gdata->drawref, 0, 0);
    g_signal_connect(gdata->drawref, "expose-event", G_CALLBACK(graph_ondraw), gdata);
    
    g_signal_connect(gdata->ref, "size-allocate", G_CALLBACK(graph_resize), gdata);    
    ...
    for (int i=0; i < 12; i++) {
        ...
        gdata->points[i].v = &(gdata->points[i]._v_h);

        gdata->points[i].t = start + 3600 * i;
        char labelbuf[8]; snprintf(labelbuf, 8, "%02ld:00", ((gdata->points[i].t) % 86400) / 3600);
        ...
        gdata->points[i].sliderref = kindle_slider_new(&(gdata->points[i]), slider_update, slider_release, 1);
        gdata->points[i].sliderref->value = *gdata->points[i].v;
        
        gtk_fixed_put(GTK_FIXED(gdata->ref), gdata->points[i].sliderref->drawing_area, 0, 1);
	}
	
    pango_font_description_free(font_desc);
    font_desc = pango_font_description_from_string(FONT_12);
	gdata->label_a = table_wrapping(button_widget((char*) "value", button_callback_a, gdata));
	gtk_widget_modify_font(gdata->label_a, font_desc);
	gtk_fixed_put(GTK_FIXED(gdata->ref), gdata->label_a, refw(gdata), refh(gdata));
	gdata->label_r = table_wrapping(button_widget((char*) "hue", button_callback_r, gdata));
	gtk_widget_modify_font(gdata->label_r, font_desc);
	gtk_fixed_put(GTK_FIXED(gdata->ref), gdata->label_r, refw(gdata), refh(gdata));
	gdata->label_g = table_wrapping(button_widget((char*) "sat", button_callback_g, gdata));
	gtk_widget_modify_font(gdata->label_g, font_desc);
	gtk_fixed_put(GTK_FIXED(gdata->ref), gdata->label_g, refw(gdata), refh(gdata));
	pango_font_description_free(font_desc);
