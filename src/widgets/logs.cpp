...
gboolean _update_log_display(void *data_vp) {
  ...
  for (int i = nlogs; i < MAX_RECORDS; i++) {
    gtk_widget_set_visible(views[i], FALSE);
  }
...
void dumplog_handler(GtkButton *_button, GdkEvent *_event, void *_data) {
  ...
  char *logbuf = (char *)malloc(logbuf_size);
  memset(logbuf, 0, logbuf_size);
  char *logbuf_wp = logbuf + snprintf(logbuf, 10, "echo -e \'");
  for (int i = 0; i < nlogs; i++) {
    logbuf_wp += snprintf(
        logbuf_wp,
        (size_t)MAX(0, (ptrdiff_t)logbuf_size - (logbuf_wp - logbuf) - 34),
        "\x1b[1m\x1b[38;5;%dm %s \x1b[0m %s\n", logs[i].pri_color,
  ...
  snprintf(logbuf_wp, 34, "\' | nc %s %ld", get_attr_str("LOG_IP"),
...
GtkWidget *log_widget() {
  ...
  PangoFontDescription *font_title =
      pango_font_description_from_string(FONT_BOLD_16);
  ...
  g_timeout_add(get_attr_long("UI_UPDATE_FREQUENCY"),
                (GSourceFunc)_update_log_display, (void *)data);
  return wrapper;
}
