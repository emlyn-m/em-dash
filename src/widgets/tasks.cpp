...
gboolean tasks_update(void *data_p) {
  ...
  for (int i = 0; i < data->num_tasks; i++) {
    ...
    char *task_markup = (char *)malloc(256);
    memset(task_markup, 0, 256);
    snprintf(task_markup, 256, data->tasks[i]->completed ? "<s> %s </s>" : "%s",
             data->tasks[i]->task);
    gtk_label_set_markup(GTK_LABEL(data->task_widgets[i]), task_markup);
  }
  for (int i = data->num_tasks; i < data->max_tasks; i++) {
    gtk_widget_set_visible(data->task_widgets[i], FALSE);
  }

  return TRUE;
}

struct task_onclick_data {
  task_t *task;
  int idx;
};
void update_task(GtkButton *button, GdkEvent *event, void *data_vp) {
  ...
  data->task->tasks[data->idx]->completed =
      1 - (data->task->tasks[data->idx]->completed);
...
gboolean keyboard_onpress(kb_data_t *data, char char_val) {
  ...
  if (task_data->pending_idx < 0) {
    ...
    task_data->tasks[0]->task = old_last_taskbuf;
    memset(task_data->tasks[0]->task, 0, 256);
  ...
  if (char_val >= 0x20 && char_val <= 0x7e &&
      strlen(task_data->tasks[0]->task) < 255) {
    task_data->tasks[0]->task[strlen(task_data->tasks[0]->task)] = char_val;
  } else if (char_val == '\b') {
    task_data->tasks[0]->task[MAX(0, strlen(task_data->tasks[0]->task) - 1)] =
        0;
  }
...
GtkWidget *tasks_widget() {
  ...
  task_data->tasks =
      (task_ev_t **)malloc(task_data->max_tasks * sizeof(task_ev_t *));
  task_data->task_widgets =
      (GtkWidget **)malloc(task_data->max_tasks * sizeof(GtkWidget *));
  ...
  for (int i = 0; i < task_data->max_tasks; i++) {
    ...
    struct task_onclick_data *ocdata =
        (struct task_onclick_data *)malloc(sizeof(struct task_onclick_data));
    ...
    g_signal_connect(evbox, "button-press-event", G_CALLBACK(update_task),
                     (void *)ocdata);
  ...
  GtkWidget *keyboard_listener =
      keyboard_widget((void *)task_data, keyboard_onpress, keyboard_onenter);
  gtk_box_pack_start(GTK_BOX(wrapper), keyboard_listener, TRUE, TRUE,
                     10 * SCALE);

  gtk_container_set_border_width(GTK_CONTAINER(wrapper), 20 * SCALE);
  g_timeout_add(get_attr_long("UI_UPDATE_FREQUENCY"), (GSourceFunc)tasks_update,
                task_data);
