...
gboolean update_weather_display(gpointer *weather_vp) {
  ...
  const int WMO_RAINY[15] = {45, 48, 51, 53, 55, 56, 57, 61,
                             63, 65, 66, 67, 80, 81, 82};
  const int WMO_THUNDER[3] = {95, 96, 99};

  weather_t *weather_data = (weather_t *)weather_vp;
  for (uint32_t event_idx = 0; event_idx < weather_data->num_weather_events;
       event_idx++) {
    ...
    if (wmo_compare(event->wmo_code, WMO_SUNNY, 2)) {
      set_image_src(GTK_IMAGE(event->widget_icon), sun, icon_width,
                    icon_height);
    } else if (wmo_compare(event->wmo_code, WMO_CLOUDY, 2)) {
      set_image_src(GTK_IMAGE(event->widget_icon), cloud, icon_width,
                    icon_height);
    } else if (wmo_compare(event->wmo_code, WMO_RAINY, 15)) {
      set_image_src(GTK_IMAGE(event->widget_icon), rain, icon_width,
                    icon_height);
    } else if (wmo_compare(event->wmo_code, WMO_THUNDER, 3)) {
      set_image_src(GTK_IMAGE(event->widget_icon), thunder, icon_width,
                    icon_height);
    }
  }
  return TRUE;
...
GtkWidget *weather_widget() {
  ...
  weather->update_freq = get_attr_long("WEATHER_UPDATE_FREQUENCY");
  weather->events = (weather_ev_t **)malloc(weather->num_weather_events *
                                            sizeof(weather_ev_t *));
  ...
  g_timeout_add(get_attr_long("NET_UPDATE_FREQUENCY"),
                (GSourceFunc)update_weather, weather);
  g_timeout_add(get_attr_long("UI_UPDATE_FREQUENCY"),
                (GSourceFunc)update_weather_display, weather);
